...
export default function Home() {
  ...
  useEffect(() => {
    // Check if we're already on a dashboard/protected route to prevent redirect loops
    ...
    if (session?.user) {
      ...
      console.log('Auth check - NextAuth role:', role);
    ...
    // Check for custom auth token if NextAuth session is not available
    // Skip if already on a dashboard/protected page
    if (pathname === '/dashboard' || 
        pathname === '/appointments' ||
        pathname === '/profile' ||
        pathname === '/records' ||
    ...
    if (token) {
      ...
      if (decoded && decoded.exp > Date.now() / 1000) {
        ...
        console.log('Auth check - JWT token role:', role);
