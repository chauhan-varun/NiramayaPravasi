...
export function verifyToken(token) {
  try {
    // Print the JWT_SECRET for debugging (first 3 chars only for security)
    console.log('JWT_SECRET prefix:', JWT_SECRET ? JWT_SECRET.substring(0, 3) + '...' : 'undefined');
    console.log('Verifying token:', token.substring(0, 15) + '...');
    
    const decoded = jwt.verify(token, JWT_SECRET);
    console.log('Token decoded successfully:', decoded.role);
    return decoded;
  } catch (error) {
