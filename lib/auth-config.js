      ...
      async authorize(credentials) {
        ...
        try {
          if (credentials.loginType === 'email') {
            ...
            // Only allow email login for SuperAdmin, Admin, and Doctor
            if (!['SUPERADMIN', 'ADMIN', 'DOCTOR'].includes(user.role)) {
              return null
            }
          ...
          if (credentials.loginType === 'phone_password') {
            // Phone + Password login (for Patient)
            const user = await prisma.user.findUnique({
              where: { phone: credentials.phone }
            })
            ...
            // Only allow for patients with this method
    ...
    async signIn({ user, account, profile }) {
      if (account?.provider === 'google') {
        ...
        if (existingUser) {
          ...
          const adminEmails = [
            'admin@example.com',
            'chauhan.varun@gmail.com',
            'varunrajput6988@gmail.com',
